...
export const startConversation = async (token: string, recipientId: string) => {
    const response = await axiosInstance.post("/api/messages/conversations/start", { recipientId }, {
        headers: { Authorization: `Bearer ${token}` },
    });
    return response.data;
};

export const sendMessage = async (token: string, data: { receiverId?: string; content: string; conversationId?: string }) => {
