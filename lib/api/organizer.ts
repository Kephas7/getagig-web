...
export const requestOrganizerVerification = async (token: string) => {
  const response = await axiosInstance.patch(
    API.ORGANIZER.REQUEST_VERIFICATION,
    {},
    {
      headers: { Authorization: `Bearer ${token}` },
    },
...
export const uploadOrganizerMedia = async (
  token: string,
  endpoint: string,
  fieldName: string,
  files: File | File[],
...
export const deleteOrganizerMedia = async (
  token: string,
  endpoint: string,
  url: string,
) => {
