...
import { MessageSquare, Search, Clock, ArrowRight, Loader2, User } from "lucide-react";
                      ...
                      <div className="flex items-center gap-6">
                        <div className="h-16 w-16 rounded-2xl bg-secondary flex items-center justify-center border-2 border-background shadow-sm overflow-hidden group-hover:scale-105 transition-transform">
                          {otherParticipant?.profilePicture ? (
                            <img src={resolveMediaUrl(otherParticipant.profilePicture)} alt={otherParticipant.username} className="h-full w-full object-cover" />
                          ) : (
                            <User size={30} className="text-muted-foreground" />
                          )}
                        </div>
                        <div>
                          <h3 className="text-lg font-black text-foreground">{otherParticipant?.username || "Unknown user"}</h3>
